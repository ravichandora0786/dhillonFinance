...
const fields = [
  ...
  {
    name: PermissionFields.DESCRIPTION,
...
const AddEditPermission = ({
  ...
  const initialValues = fields.reduce((acc, f) => {
    if (f.name === PermissionFields.STATUS) {
      acc[f.name] = "Inactive";
    } else {
      acc[f.name] = "";
    }
        ...
        dispatch(
          createPermission({
            ...
            onSuccess: (response) => {
              ...
              callBackFunc(permissionSearchData, pagination);
              onBack();
            },
            onFailure: (error) => {
  ...
  useEffect(() => {
    if (Object.keys(data).length > 0) {
      setInitialObject({
        [PermissionFields.ID]: data[PermissionFields.ID],
        [PermissionFields.NAME]: data[PermissionFields.NAME],
        [PermissionFields.DESCRIPTION]: data[PermissionFields.DESCRIPTION],
        [PermissionFields.STATUS]: data[PermissionFields.STATUS],
      });
    } else {
  ...
  return (
    <GenericModal
      showModal={openModal}
      closeModal={onBack}
      modalTitle={isEdit ? "Edit Permission" : "Add New Permission"}
      modalBody={
        <Formik
